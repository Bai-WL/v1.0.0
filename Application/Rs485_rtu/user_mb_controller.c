...
static uint32_t reg_cache[CACHE_SIZE];
...
ReqCreat_State MBController_Request(MB_Request req)
{
    ...
    if (req.type == REQ_WRITE_SINGLE_COIL ||
    ...
    else if (req.type == REQ_READ_COILS ||
             req.type == REQ_READ_HOLDING_REGISTERS)
    {
        // 添加到普通队列（读请求）
...
bool MBController_GetCache(uint32_t reg_addr, uint16_t *value)
{
    ...
    {
        if ((reg_cache[i] >> 16) == reg_addr)
        {
            *value = reg_cache[i] & 0xFFFF;
            return true;
        }
    }
    return false;
...
void MBController_Process(BSP_ModbusHandleTypedef *_hModbus)
{
    ...
    case MB_WRITING:
    {
        ...
        // 复制写入数据
        if (current_req.data != NULL && current_req.len_of_data > 0)
        {
            write_len = (current_req.len_of_data > 16) ? 16 : current_req.len_of_data;
            for (uint8_t i = 0; i < write_len; i++)
            {
                write_data[i] = current_req.data[i];
            }
        }
        ...
        case REQ_WRITE_SINGLE_COIL: // 功能码 0x05
            ...
            if (write_len > 0)
            {
                ...
                status = bsp_MBMasterTransmitCommand(_hModbus, current_req.slave_addr,
                                                     0x05, current_req.reg_addr, write_data, 1, &mb_error,
        ...
        case REQ_WRITE_SINGLE_REGISTER: // 功能码 0x06
            ...
            {
                status = bsp_MBMasterTransmitCommand(_hModbus, current_req.slave_addr,
                                                     0x06, current_req.reg_addr, write_data, 1, &mb_error,
                                                     write_data, &rx_data_len);
            }
            break;
        ...
        case REQ_WRITE_MULTIPLE_REGISTERS: // 功能码 0x10
            ...
            {
                status = bsp_MBMasterTransmitCommand(_hModbus, current_req.slave_addr,
                                                     0x10, current_req.reg_addr, write_data, write_len, &mb_error,
                                                     write_data, &rx_data_len);
            }
            break;
    ...
    case MB_READING:
    {
        ...
        case REQ_READ_COILS: // 功能码 0x01
            status = bsp_MBMasterTransmitCommand(_hModbus, current_req.slave_addr,
                                                 0x01, current_req.reg_addr, NULL, current_req.data_len, &mb_error,
                                                 rx_buffer, &rx_data_len);
            break;

        case REQ_READ_HOLDING_REGISTERS: // 功能码 0x03
            status = bsp_MBMasterTransmitCommand(_hModbus, current_req.slave_addr,
                                                 0x03, current_req.reg_addr, NULL, current_req.data_len, &mb_error,
                                                 rx_buffer, &rx_data_len);
            break;
    ...
    case MB_WAITING_RSP:
    {
        ...
        if (mb_error != MODBUS_BUSY)
        {
            ...
            case MODBUS_SUCESS:
                    ...
                    current_req.type == REQ_READ_HOLDING_REGISTERS)
                {
                    ...
                    if (current_req.type == REQ_READ_COILS)
                    {
                        ...
                        for (uint16_t i = 0; i < current_req.data_len; i++)
                        {
                            ...
                            if (byte_index < rx_data_len)
                            {
                                ...
                                // 更新装置存储（位装置）
                                if (cache_addr <= BIT_DEVICE_COUNT)
                                {
                                    DeviceStorage_SetBit(cache_addr, (coil_value != 0));
                                }
                    ...
                    else // REQ_READ_HOLDING_REGISTERS
                    {
                        ...
                        for (uint8_t i = 0; i < rx_data_len; i++)
                        {
                            ...
                            // 更新装置存储（字装置）
                            if (cache_addr >= WORD_DEVICE_START && cache_addr <= WORD_DEVICE_END)
                            {
                                DeviceStorage_SetWord(cache_addr, rx_buffer[i]);
                            }
                        }
                    }
                ...
                {
                    if (current_req.data != NULL)
                    {
                        free((void *)current_req.data);
                        current_req.data = NULL;
                    }
                }
                // 重置状态
                mb_state = MB_IDLE;
            ...
            case MODBUS_ERRACK: // 从机错误应答
                ...
                if (timeoutcnt > 5)
                {
                    rs_err = (mb_error == MODBUS_NACK) ? 1 : (mb_error == MODBUS_ACKERR) ? 2
                                                                                         : 3;
                    mb_state = MB_IDLE;
                }
                else
