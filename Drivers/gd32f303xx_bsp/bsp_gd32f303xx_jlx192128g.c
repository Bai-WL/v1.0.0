...
#define JLX_SPI_RST_GPIO_CLK_ENABLE() do { rcu_periph_clock_enable(RCU_GPIOB); } while(0)

#define JLX_SPI_RS_GPIO_CLK_ENABLE() do { rcu_periph_clock_enable(RCU_GPIOB); } while(0)

#define JLX_SPI_CS_GPIO_CLK_ENABLE() do { rcu_periph_clock_enable(RCU_GPIOA); } while(0)

#define JLX_BL_GPIO_CLK_ENABLE() do { rcu_periph_clock_enable(RCU_GPIOC); } while(0)

// @todo 需要编写通配符数据显示函数
#define SPI0_DATA_ADDRESS 0x4001300C

#define JLX_SPI_RST(x)  do { x ? \
                            gpio_bit_set(JLX_SPI_RST_GPIO_PORT, JLX_SPI_RST_GPIO_PIN) : \
                            gpio_bit_reset(JLX_SPI_RST_GPIO_PORT, JLX_SPI_RST_GPIO_PIN) ;\
                        } while(0)
#define JLX_SPI_RS(x)   do { x ? \
                            gpio_bit_set(JLX_SPI_RS_GPIO_PORT, JLX_SPI_RS_GPIO_PIN)   : \
                            gpio_bit_reset(JLX_SPI_RS_GPIO_PORT, JLX_SPI_RS_GPIO_PIN) ; \
                        } while(0)
#define JLX_SPI_CS(x)   do { x ? \
                            gpio_bit_set(JLX_SPI_CS_GPIO_PORT, JLX_SPI_CS_GPIO_PIN)   :\
                            gpio_bit_reset(JLX_SPI_CS_GPIO_PORT, JLX_SPI_CS_GPIO_PIN) ;\
                        } while(0)
#define JLX_BL(x)   do { x ? \
                            gpio_bit_set(JLX_BL_GPIO_PORT, JLX_BL_GPIO_PIN)   :\
                            gpio_bit_reset(JLX_BL_GPIO_PORT, JLX_BL_GPIO_PIN) ;\
                        } while(0)
...
void bsp_JLXLcdClearRect(uint8_t _ucx, uint8_t _ucxCnt, uint8_t _ucy, uint8_t _ucyCnt, uint8_t _ucColor)
{
    int i, j;
    #ifdef JLX_SPIDMA
    for (i = 0; i < _ucyCnt && (_ucy + i) < JLXLCD_H/8; i++) {      /* 数组边界保护 */
        for (j = _ucx; j < _ucxCnt+_ucx && j < JLXLCD_W ; j++) {    /* 数组边界保护 */
...
void bsp_JLXLcdClearLine(uint8_t _ucy, uint8_t _ucyCnt, uint8_t _ucColor)
{
    int i, j;
    #ifdef JLX_SPIDMA
    for (i = 0; i < _ucyCnt && (_ucy + i) < JLXLCD_H/8; i++) {	/* 数组边界保护 */ /* FIXME: 紧急bug修复, 此处数组越界 */
        for (j = 0; j < 192; j++) {
            frame_buffer[(_ucy+i)*192 + j] = _ucColor;
        }
    }
